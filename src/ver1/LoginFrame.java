...
public class LoginFrame extends JFrame {
	...
	public LoginFrame(Client client,ClientFrame mContext ) {
			...
			public void mouseReleased(MouseEvent e) {
				if (isLoginButton(e.getX(), e.getY())) {
					...
					if (name.isEmpty()) {
						...
						(new WaitingRoom(react, client)).setTitle("메모장 -" + name);
						setVisible(false);
						mContext.setVisible(false);
						client.clickConnectServerBtn( "192.168.0.127", 10000, name);
					}
				}
			}
		});
